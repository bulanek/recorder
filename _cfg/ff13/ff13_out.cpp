...
#define	MMC_CD		(GPIOC->IDR & GPIO_IDR_ID4 )	/* Card detect (yes:true, no:false, default:true) */
...
#define	MMC_WP		0 /* Write protected (yes:true, no:false, default:false) */
#define SPIx_CR1	SPI2->CR1
#define SPIx_SR		SPI2->SR
#define SPIx_DR		SPI2->DR
#define	SPIxENABLE() {\
...
//#define CS_HIGH()	do{SPI3->CR1 |= SPI_CR1_SSI;}while(0)
//#define CS_LOW()	do{SPI3->CR1 &= ~SPI_CR1_SSI;}while(0)

#define	MMC_CD		1 //Note BoBu not implemented (GPIOC->IDR & GPIO_IDR_ID4 )	/* Card detect (yes:true, no:false, default:true) */
#define	MMC_WP		0 /* Write protected (yes:true, no:false, default:false) */
#define SPIx_CR2	(SPI3->CR2)
#define SPIx_CR1	(SPI3->CR1)
#define SPIx_SR		(SPI3->SR)
#define SPIx_DR		(SPI3->DR)

#define SPIxENABLE() NV_INIT()
#endif
...
{
    IRQn_Type type = DMA1_Stream5_IRQn;
    NVIC_EnableIRQ(type);
    NVIC_SetPriority(type, 15U);

    RCC->AHB1ENR |= RCC_AHB1ENR_DMA1EN;

    DMA1_Stream5->CR &= ~DMA_SxCR_EN;			/*the software is allowed to program the Configuration and FIFO bits registers. */
    while ((DMA1_Stream5->CR & DMA_SxCR_EN) != 0);

    DMA1_Stream5->PAR = (__IO uint32_t )&(SPIx_DR);
    DMA1_Stream5->M0AR = (uint32_t)0;
	DMA1_Stream5->NDTR = 0;

	DMA1_Stream5->CR &= ~DMA_SxCR_CHSEL ;  /* Channel 0 (SPI3_Tx) */

    DMA1_Stream5->CR |=  DMA_SxCR_PL_0 | DMA_SxCR_PL_1;     /* Priority very high*/
	DMA1_Stream5->FCR &= ~DMA_SxFCR_DMDIS;	/* Direct mode enable */

    DMA1_Stream5->CR |= DMA_SxCR_DIR_0;		/* Memory-to-peripheral*/

    DMA1_Stream5->CR &= ~DMA_SxCR_MSIZE;	/* 8 bit size of memory */
    DMA1_Stream5->CR &= ~DMA_SxCR_PSIZE;	/* 8 bit size of peripheral. */
    ...
    DMA1_Stream5->CR |= DMA_SxCR_DMEIE;		/* Direct mode error  */
    DMA1_Stream5->FCR |= DMA_SxFCR_FEIE;	/* FiFo error  */
...
{
	ASSERT(((SPIx_CR2 & SPI_CR2_TXDMAEN) != 0), "fatal, not enabled dma on spi");
	ASSERT((SPIx_CR1 & SPI_CR1_SPE) != 0,"fatal, spi not enabled");
	volatile uint8_t data = SPIx_DR;

    IRQn_Type type = DMA1_Stream5_IRQn;
    NVIC_EnableIRQ(type);
    NVIC_SetPriority(type, 15U);

	ASSERT(((DMA1_Stream5->CR & DMA_SxCR_EN) == 0), "fatal, enabled dma");
    DMA1_Stream5->M0AR = (uint32_t)buff;
    DMA1_Stream5->NDTR = btx;
    DMA1_Stream5->CR |= DMA_SxCR_EN;

    vTaskSuspend(tskma_get_nv_task_handle());
	while ((SPIx_SR & (SPI_SR_TXE| SPI_SR_BSY)) != SPI_SR_TXE) ;	/* Wait for end of the SPI transaction */
	while ((DMA1_Stream5->CR & DMA_SxCR_EN) != 0);
	data = SPIx_DR;
}
#else
